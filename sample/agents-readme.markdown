...
- Future human-gated stage contract (proposed): `docs/specs/phase1/human_required_stage_contract.md`
...
- `--run-config-file` to merge additional run config from a JSON object file.
...
## Watch inbox and auto-trigger runs

Watcher mode scans filesystem inbox bundles and executes runs automatically.
...
inbox/<pipeline_id>/<bundle_id>/
  ...
  payload/
  _READY                 # required completeness marker
```

Run once (cron/Task Scheduler friendly):

```bash
seedpipe-watch --once --pipeline all
```

Long-running poller:

```bash
seedpipe-watch --pipeline all --poll-seconds 5
```

Key behaviors:
- claims bundles atomically into `inbox/<pipeline_id>/.claimed/`.
- snapshots payload into `artifacts/inputs/<run_id>/`.

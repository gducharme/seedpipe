...
def run(run_config: dict[str, object], attempt: int = 1) -> int:
    ...
    while True:
        ...
        if (not loop_continue) and (0 >= cycle_start_index) and (PIPELINE_TYPE == 'looping' or _should_run_stage(manifest=manifest, stage_id='ingest', stage_index=0, resume_index=resume_index)):
            _mark_stage(manifest, stage_id='ingest', status='running', attempt=attempt)
            try:
                ...
                _register_stage_outputs(manifest=manifest, stage_id='ingest', loop_iteration=loop_iteration, outputs=['items.jsonl'])
                run_config['_artifact_index'] = _artifact_index(manifest)
                ctx_base = StageContext.make_base(run_config=run_config)
                _mark_stage(manifest, stage_id='ingest', status='completed', attempt=attempt)
            except Exception as exc:
                _mark_stage(manifest, stage_id='ingest', status='failed', attempt=attempt, error={'message': str(exc)})
                raise
        if (not loop_continue) and (1 >= cycle_start_index) and (PIPELINE_TYPE == 'looping' or _should_run_stage(manifest=manifest, stage_id='transform', stage_index=1, resume_index=resume_index)):
            _mark_stage(manifest, stage_id='transform', status='running', attempt=attempt)
            try:
                ...
                if stage_failed_item_ids:
                    loop_target = _resolve_loop_target('transform')
                    if PIPELINE_TYPE == 'looping' and loop_target is not None:
                        ...
                        loop_continue = True
                        loop_origin_stage = 'transform'
                    else:
                        raise RuntimeError(f'stage transform failed for items: {sorted(set(stage_failed_item_ids))}')
                _register_stage_outputs(manifest=manifest, stage_id='transform', loop_iteration=loop_iteration, outputs=['transformed.jsonl'])
                run_config['_artifact_index'] = _artifact_index(manifest)
                ctx_base = StageContext.make_base(run_config=run_config)
                _mark_stage(manifest, stage_id='transform', status='completed', attempt=attempt)
            except Exception as exc:
                _mark_stage(manifest, stage_id='transform', status='failed', attempt=attempt, error={'message': str(exc)})
                raise
        if (not loop_continue) and (2 >= cycle_start_index) and (PIPELINE_TYPE == 'looping' or _should_run_stage(manifest=manifest, stage_id='validate', stage_index=2, resume_index=resume_index)):
            _mark_stage(manifest, stage_id='validate', status='running', attempt=attempt)
            try:
                ...
                _register_stage_outputs(manifest=manifest, stage_id='validate', loop_iteration=loop_iteration, outputs=['validation_report.json'])
                run_config['_artifact_index'] = _artifact_index(manifest)
                ctx_base = StageContext.make_base(run_config=run_config)
                _mark_stage(manifest, stage_id='validate', status='completed', attempt=attempt)
            except Exception as exc:
                _mark_stage(manifest, stage_id='validate', status='failed', attempt=attempt, error={'message': str(exc)})
                raise
        if (not loop_continue) and (3 >= cycle_start_index) and (PIPELINE_TYPE == 'looping' or _should_run_stage(manifest=manifest, stage_id='future_review', stage_index=3, resume_index=resume_index)):
            ...
            try:
                ...
                _register_stage_outputs(manifest=manifest, stage_id='future_review', loop_iteration=loop_iteration, outputs=['reviewed_report.json'])
                run_config['_artifact_index'] = _artifact_index(manifest)
                ctx_base = StageContext.make_base(run_config=run_config)
                ...
                raise
        if (not loop_continue) and (4 >= cycle_start_index) and (PIPELINE_TYPE == 'looping' or _should_run_stage(manifest=manifest, stage_id='publish', stage_index=4, resume_index=resume_index)):
